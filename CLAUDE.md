...
### 基线（2026-09-01，语料 `test/科研中文.txt` 1552 字）

| 配置 | 首选命中 | 偏移加权 | 选择数 | 达成率@400kpm |
|---|---|---|---|---|
| 上游起点（0.6B） | 38.5% | 575 | 400 | 51.3% |
| 换 1.7B | 47.1% | 391 | 333 | 60.4% |
| + 长候选首屏配额 | 47.1% | 364 | 333 | 62.0% |
| **+ 预置 79 个领域词** | **51.5%** | **201** | **272** | **74.4%** |

总选择成本累计降 65%，且最后一档把偏移 ≥10 的长尾**清零**（最大偏移 7）。

**收益最大的一项是补领域词库，不是换模型、也不是改排序** —— 但它只在用户词有了真实分数之后才生效：上游那版用户词分数恒为 0，词加进去也赢不了候选。
...
| 无 | 47.1% | 364 |
| **79 个手挑领域词** | **51.5%** | **201** |
| 2 万通用词（全给优先） | 41.1% | 353 |
| 2 万通用词（不给优先）+ 领域词 | 41.7% | 343 |

**灌 2 万通用词比不加词库还差。** 拆出 `vouchedTokens`（只有用户亲自确认的词享受同长度优先）只把 353 救到 343，离 201 差得远 —— 说明伤害不只来自优先权：这些词**光是存在于候选池里**就够了，它们提供大量同音多字候选，在「长度优先」这个主排序键上把正确的短答案顶下去。

所以**不要 bulk import**。`LIME_BULK_WORDS` 存在只是为了让批量导入不至于更糟，不是推荐用法。扩词库的正路是 `LIME_RECORD` 记录真实输入 + `preload_word.ts text` 提词，产出小而准的个人词表。

### 速度

输入法的硬约束是每敲一键要多久出候选，`test_text.ts` 不测这个，用 `bench_latency.ts`。

| 项 | 耗时 |
|---|---|
| 引擎出候选（1.7B + GPU，p50 / p90） | 59 / 106 ms |
| 换 0.6B | 只快 6ms，不值得丢掉 32% 的成本下降 |
| 两万用户词对延迟的影响 | p50 59→62ms（`last_result` 本就有约 15 万真实 token） |
| **RIME 的 HiAE 加密（每键 2 次 deno 冷启动）** | **约 128 ms** |
| curl 进程 + HTTP | 约 20–50 ms |

**瓶颈是加密层不是模型。** `rime/lua/llm_pinyin.lua` 第 39 / 56 行各 `io.popen` 起一个 deno 进程，实测冷启动中位数 64ms。本机自用把 `enable_hiae` 改 `false` 可砍掉约 128ms。

**语料是 Claude 写的，不是用户真实写作**，LLM 写的文本对 LLM 更可预测，绝对值可能偏乐观；这些数字之间的**相对比较**才是可信的。另外这套指标与万象拼音的句/字准确率**不可比**。
